...
public class GensHatchEggListener {

    @SubscribeEvent
    public void onHatch (PlayerEggHatchEvent event) {

        EntityPlayerMP player = event.getPlayer();
        NBTTagCompound eggNBT = event.getEggNBT();
        EntityPixelmon p = new EntityPixelmon(player.world);
        p.readFromNBT(eggNBT);
        ...
        if (SkillGetters.hatchingBlacklist.contains(p.getPokemonName())) return;
        ...
        if (SkillGetters.breederHatchingModifiers.containsKey("Alolan")) {
            ...
            if (mod > 0) {
                ...
                if (PixelSkills.alolans.contains(p.getPokemonName()) && p.getForm() == 3) {
        ...
        if (SkillGetters.breederHatchingModifiers.containsKey("Alpha")) {

            double mod = SkillGetters.breederHatchingModifiers.get("Alpha");
            if (mod > 0) {
        ...
        if (SkillGetters.breederHatchingModifiers.containsKey("Galarian")) {
            ...
            if (mod > 0) {
                ...
                if (PixelSkills.galarians.contains(p.getPokemonName()) && p.getForm() == 10) {
        ...
        if (SkillGetters.breederHatchingModifiers.containsKey("Hisuian")) {
            ...
            if (mod > 0) {
                ...
                if (PixelSkills.hisuians.contains(p.getPokemonName()) && p.getForm() == 11) {
